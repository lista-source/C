...
class FirebaseRepository @Inject constructor(
    ...
    private val MAX_DISTANCE_METERS = 500.0   // relaxed for testing
    ...
    suspend fun markAttendanceManual(
        ...
        courseId: String,
        lecturerId: String
    ): Result<Unit> = markAttendanceRecord(studentId, courseId, VerificationMethod.MANUAL, "", lecturerId)

    private suspend fun markAttendanceRecord(
        studentId: String,
        courseId: String,
        ...
        return try {
            val key = database.getReference("attendance").push().key
                ?: throw Exception("DB key generation failed")
    ...
    /** Lecturer: get all attendance for a specific course */
    suspend fun getAttendanceByCourse(courseId: String): List<Attendance> {
        return try {
            val snapshot = database.getReference("attendance")
    ...
    suspend fun getUsersByRole(role: UserRole): List<User> {
        return try {
            val snapshot = database.getReference("users")
                .orderByChild("role").equalTo(role.name).get().await()
            snapshot.children.mapNotNull { it.getValue(User::class.java) }
        } catch (e: Exception) { emptyList() }
    }

    // ── Chat — PRIVATE conversations ──────────────────────────────────────
    // Path: chats/{conversationId}/{messageId}
    // conversationId = sorted UIDs joined with "_" → only two people can see it
    ...
    suspend fun sendMessage(message: ChatMessage): Result<Unit> {
        return try {
            ...
            val key = database.getReference("chats/$convId").push().key
                ?: throw Exception("DB key generation failed")
            database.getReference("chats/$convId/$key")
                .setValue(message.copy(id = key)).await()
            Result.success(Unit)
        } catch (e: Exception) { Result.failure(e) }
    }

    /** Returns the Firebase path for a private conversation (used by ChatScreen) */
    fun getConversationPath(uid1: String, uid2: String): String =
        "chats/${conversationId(uid1, uid2)}"
